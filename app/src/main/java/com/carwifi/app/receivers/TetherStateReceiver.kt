 ...
 * - 文件共享服务器随热点生命周期协调起停（热点开+开关开 → 运行，否则停止）。
 *
...
class TetherStateReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(ctx: Context, intent: Intent?) {
        ...
        scope.launch {
            ...
            val hotspotOn = t.isHotspotOn()

            // 文件共享随热点自动开关（无论热点由本应用还是系统自动任务管理）
            FileShareManager.applyFromSettings(ctx, s, hotspotOn)

            // 热点保活（仅本应用接管且充电中时）
            if (!s.tetheringEnabled || !HotspotPolicy.shouldControlByApp(s) || !BatteryUtils.isCharging(ctx)) return@launch
            if (hotspotOn) return@launch
            ...
            AuditLogger(ctx.filesDir).log(
                ...
                else "热点状态变更：应用无法自动开热点，请在系统设置手动开启"
            )
        }
    }
