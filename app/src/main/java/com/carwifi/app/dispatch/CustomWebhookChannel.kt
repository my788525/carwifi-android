...
class CustomWebhookChannel(override val config: ChannelConfig) : Channel {
            ...
            val code = when (config.method.uppercase()) {
                "GET" -> Http.get(config.url)
                    ...
                    if (rendered.trim().startsWith("{")) {
                        Http.postJson(config.url, rendered)
                    } else {
                        Http.postRaw(config.url, rendered, "text/plain; charset=utf-8")
                    }
                }
            }
